...
- EYEKONS Stan 개인정보 처리방침은 `/apps/eyecons-stan/privacy/` 영문 기본 경로와 `/ko/`, `/ja/` 번역 경로를 유지한다. `/en/`과 기존 `/apps/eyecons/privacy/` 경로는 호환용으로 보존하며, 세 언어의 사실관계와 운영자 보호 범위를 동일하게 동기화한다.
